  ...
  return (
    <g>
      ...
      <ellipse
        ...
        rx="14"
        ry="6"
        transform={`rotate(${region.id === "vehicle" ? -24 : -35} ${midX} ${midY})`}
      />
    </g>
  );
}
...
function DynamicObject({ region, onSelect }) {
  const isVehicle = region.id === "vehicle";
  ...
  return (
    ...
    >
      ...
      <path
        ...
        d={`M ${position.x} ${position.y} l ${region.velocity.x * 80} ${region.velocity.y * 80}`}
        strokeWidth="0.75"
      />
      <text className="fill-cyan-50 text-[3px] [paint-order:stroke] [stroke:rgba(0,0,0,0.55)] [stroke-width:0.45]" x={position.x + 5} y={position.y - 5}>
        {isVehicle ? `Vehicle ${region.speedMps.toFixed(1)}m/s` : `Pedestrian ${region.speedMps.toFixed(1)}m/s`}
      </text>
    </motion.g>
  );
}
    ...
    <div className="panel">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          title="Toggle simulated LiDAR point cloud"
          type="button"
        >
          LiDAR
        </button>
